...
treatGPIO = configParser.get('feederConfig', 'Treat_Button_GPIO_Pin')
hopperGPIO = configParser.get('feederConfig', 'Hopper_GPIO_Pin')
hopperTime = configParser.get('feederConfig', 'Hopper_Spin_Time')
LOG_TreatService_FILENAME = configParser.get('feederConfig', 'Log_TreatService_Filename')
...
handler = logging.handlers.TimedRotatingFileHandler(LOG_TreatService_FILENAME, when="midnight", backupCount=3)
...
treatButton = int(treatButtonGPIO)
GPIO.setmode(GPIO.BOARD)
GPIO.setwarnings(False)
GPIO.cleanup(treatButton)
