...
treatGPIO = str(configParser.get('feederConfig', 'Treat_Button_GPIO_Pin'))
...
def feedbuttonclickSmartHome():
    try:
        ...
        dbInsert = commonTasks.db_insert_feedtime(dataNewObject,4)
        if dbInsert != 'ok':
            flash('Warning. feedtime did not update: ' + str(updatescreen), 'warning')
...
@app.route('/treatbuttonclickSmartHome', methods=['GET', 'POST'])
...
def admin_page():
    try:
        if 'userLogin' in session:
            ...
            treatServiceFullOutput = ControlService('feederTreatService', 'status')
            treatServiceFinalStatus = CleanServiceStatusOutput(str(treatServiceFullOutput))
            ...
            return render_template('admin.html'
                                   ...
                                   , treatServiceFinalStatus=treatServiceFinalStatus
...
@app.route('/startTreatService', methods=['GET', 'POST'])
def startTreatService():
    try:
        if 'userLogin' in session:
            myLogTimeServiceFullOutput = ControlService('feederTreatService', 'start')

            flash('Treat Service Started!')
            return redirect(url_for('admin_page'))
        else:
            return redirect(url_for('admin_login_page'))
    except Exception as e:
...
@app.route('/stopTreatService', methods=['GET', 'POST'])
def stopTreatService():
    try:
        if 'userLogin' in session:
            myLogTimeServiceFullOutput = ControlService('feederTreatService', 'stop')

            flash('Treat Service Stopped!')
            return redirect(url_for('admin_page'))
        else:
            return redirect(url_for('admin_login_page'))
    except Exception as e:
